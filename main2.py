...
@bot.on_message(filters.command(["bot"])&   (filters.chat(sudo_groups)))
async def account_login(bot: Client, m: Message):
    ...
    try:
        for i in range(arg, len(links)):
            ...
            elif "d3nzo6itypaz07" in url:
                cmd = f'yt-dlp -f "{ytf}" --hls-prefer-ffmpeg --no-keep-video --remux-video mkv --referer "https://www.pw.live" "{url}" -o "{name}.mkv" --no-warnings --socket-timeout 30'
    ...
    try:
        for count, i in enumerate(range(int(raw_text) - 1, len(content)),
            ...
            cook, url = requests.get(
                f"http://api.newdomainhai.gq/gurukul?link={link}").json().values()
...
@bot.on_message(filters.command(["class"])&   (filters.chat(sudo_groups)))
async def account_login(bot: Client, m: Message):
    editable = await m.reply_text(f"**Hi im classplus dl**")
    input: Message = await bot.listen(editable.chat.id)
    x = await input.download()
    ...
    try:
        for count, i in enumerate(range(int(raw_text) - 1, len(content)),
            ...
            name1, link = content[i].split(":", 1)
            cook, url = requests.get(
                f"https://styextract.vercel.app/cp?link={link}").json().values()
